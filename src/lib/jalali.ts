// Jalali (Persian Solar) date conversion and formatting utilities
// Algorithm based on the algorithm by Kazimierz M. Borkowski

const persianMonths = [
  'فروردین', 'اردیبهشت', 'خرداد', 'تیر', 'مرداد', 'شهریور',
  'مهر', 'آبان', 'آذر', 'دی', 'بهمن', 'اسفند',
]

const persianWeekdays = ['یکشنبه', 'دوشنبه', 'سه‌شنبه', 'چهارشنبه', 'پنجشنبه', 'جمعه', 'شنبه']

function toJalali(gy: number, gm: number, gd: number): [number, number, number] {
  const g_d_m = [0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334]
  let jy = gy <= 1600 ? 0 : 979
  gy -= gy <= 1600 ? 621 : 1600
  const gy2 = gm > 2 ? gy + 1 : gy
  let days = 365 * gy + Math.floor((gy2 + 3) / 4) - Math.floor((gy2 + 99) / 100) + Math.floor((gy2 + 399) / 400) - 80 + gd + g_d_m[gm - 1]
  jy += 33 * Math.floor(days / 12053)
  days %= 12053
  jy += 4 * Math.floor(days / 1461)
  days %= 1461
  if (days > 365) {
    jy += Math.floor((days - 1) / 365)
    days = (days - 1) % 365
  }
  const jm = days < 186 ? 1 + Math.floor(days / 31) : 7 + Math.floor((days - 186) / 30)
  const jd = 1 + (days < 186 ? days % 31 : (days - 186) % 30)
  return [jy, jm, jd]
}

export function toJalaliDate(date: Date): { year: number; month: number; day: number; weekday: string } {
  const [jy, jm, jd] = toJalali(date.getFullYear(), date.getMonth() + 1, date.getDate())
  return { year: jy, month: jm, day: jd, weekday: persianWeekdays[date.getDay()] }
}

export function formatJalaliDate(iso: string): string {
  if (!iso) return ''
  const d = new Date(iso)
  const j = toJalaliDate(d)
  return `${j.weekday} ${j.day} ${persianMonths[j.month - 1]} ${j.year}`
}

export function formatJalaliDateShort(iso: string): string {
  if (!iso) return ''
  const d = new Date(iso)
  const j = toJalaliDate(d)
  return `${j.day} ${persianMonths[j.month - 1]} ${j.year}`
}

export function formatJalaliDateTime(iso: string): string {
  if (!iso) return ''
  const d = new Date(iso)
  const j = toJalaliDate(d)
  const hh = String(d.getHours()).padStart(2, '0')
  const mm = String(d.getMinutes()).padStart(2, '0')
  return `${j.day} ${persianMonths[j.month - 1]} ${j.year} - ${hh}:${mm}`
}

export function todayJalaliShort(): string {
  const j = toJalaliDate(new Date())
  return `${j.year}/${String(j.month).padStart(2, '0')}/${String(j.day).padStart(2, '0')}`
}
